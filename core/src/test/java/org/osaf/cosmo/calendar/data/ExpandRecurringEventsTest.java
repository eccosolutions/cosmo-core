...
public class ExpandRecurringEventsTest {
    ...
    public void testExpandEvent() throws Exception {
        ...
        Assertions.assertEquals(1, calendar.getComponents().getComponents("VEVENT").size());
        ...
        Assertions.assertEquals(3, comps.size());

        Iterator<VEvent> it = comps.iterator();
        VEvent event = it.next();

        Assertions.assertEquals("event 6", event.getProperties().<Summary>getProperty(Property.SUMMARY).getValue());
        Assertions.assertEquals("20060102T190000Z", event.getStartDate().getDate().toString());
        Assertions.assertEquals("20060102T190000Z", event.getRecurrenceId().getDate().toString());

        event = it.next();
        Assertions.assertEquals("event 6", event.getProperties().<Summary>getProperty(Property.SUMMARY).getValue());
        Assertions.assertEquals("20060103T190000Z", event.getStartDate().getDate().toString());
        Assertions.assertEquals("20060103T190000Z", event.getRecurrenceId().getDate().toString());

        event = it.next();
        Assertions.assertEquals("event 6", event.getProperties().<Summary>getProperty(Property.SUMMARY).getValue());
        Assertions.assertEquals("20060104T190000Z", event.getStartDate().getDate().toString());
        Assertions.assertEquals("20060104T190000Z", event.getRecurrenceId().getDate().toString());
    ...
    public void testExpandEventWithOverrides() throws Exception {
        ...
        Assertions.assertEquals(5, comps.size());
        ...
        event = it.next();
        Assertions.assertEquals("event 6 changed", event.getProperties().<Summary>getProperty(Property.SUMMARY).getValue());
        Assertions.assertEquals("20060104T210000Z", event.getStartDate().getDate().toString());
        Assertions.assertEquals("20060104T190000Z", event.getRecurrenceId().getDate().toString());
    ...
    public void REMOVEDtestExpandNonRecurringEvent() throws Exception {
        ...
        Assertions.assertEquals("event 6", event.getProperties().<Summary>getProperty(Property.SUMMARY).getValue());
        Assertions.assertEquals("20060102T190000Z", event.getStartDate().getDate().toString());
        Assertions.assertNull(event.getRecurrenceId());
    ...
    private void verifyExpandedCalendar(Calendar calendar) {
        ...
        Assertions.assertNull(calendar.getComponents().getComponent("VTIMEZONE"));
        ...
        for(Iterator<VEvent> it = comps.iterator();it.hasNext();) {
            ...
            Assertions.assertNull(event.getStartDate().getParameters().getParameter(Parameter.TZID));
            Assertions.assertTrue(dt.isUtc());

            // verify no recurrence rules
            Assertions.assertNull(event.getProperties().getProperty(Property.RRULE));
        }
    }
